...
class SupportMatrixTests(unittest.TestCase):
    ...
    def test_rejects_unapproved_target_text(self):
        for target in (
            "x86_64-unknown-linux-gnu; echo injected",
            ...
            with self.subTest(target=target):
                invalid = copy.deepcopy(self.raw)
                invalid["targets"][0]["target"] = target

                with self.assertRaisesRegex(SystemExit, "target is not approved"):
                    parse_registry(invalid)

    def test_rejects_unapproved_runner_labels(self):
        for runner in ("self-hosted", "ubuntu-latest,privileged", "private-runner", []):
            with self.subTest(runner=runner):
                invalid = copy.deepcopy(self.raw)
                invalid["targets"][0]["ci"]["runner"] = runner

                with self.assertRaisesRegex(SystemExit, "unapproved runner"):
                    parse_registry(invalid)
    ...
    def test_rejects_runtime_generated_matrix_expression(self):
    ...
    def test_rejects_workflow_matrix_consumption_drift(self):
        ...
        invalid["jobs"]["uclibc"]["strategy"]["matrix"]["include"][0][
            "target"
        ] = "x86_64-unknown-linux-gnu"
    ...
    def test_rejects_mutable_action_references(self):
        invalid = copy.deepcopy(load_workflow())
        invalid["jobs"]["support-matrix"]["steps"][0]["uses"] = "actions/checkout@v4"

        with self.assertRaisesRegex(SystemExit, "not pinned to a commit"):
            validate_workflow(self.data, invalid)

    def test_rejects_unquoted_matrix_target_in_shell_commands(self):
        invalid = copy.deepcopy(load_workflow())
        command = invalid["jobs"]["check"]["steps"][3]["run"]
        invalid["jobs"]["check"]["steps"][3]["run"] = command.replace(
            '"${{ matrix.target }}"', "${{ matrix.target }}"
        )

        with self.assertRaisesRegex(SystemExit, "unquoted matrix target"):
            validate_workflow(self.data, invalid)

    def test_rejects_unquoted_target_after_quoted_target(self):
        invalid = copy.deepcopy(load_workflow())
        invalid["jobs"]["check"]["steps"][3]["run"] += (
            "\necho ${{ matrix.target }}"
        )
