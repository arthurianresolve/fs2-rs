...
struct PairResult {
    ...
    pair: usize,
    order: [String; 2],
    baseline: CriterionRun,
    candidate: CriterionRun,
...
impl PairRunner<'_> {
    fn execute(&self, order: PairOrder, replicate: usize, local_pair: usize) -> Result<PairResult> {
        ...
        let (baseline, baseline_valid) = observed
            .remove(&Subject::Baseline)
            .ok_or_else(|| invalid_data("missing baseline benchmark result"))?;
        let (candidate, candidate_valid) = observed
            .remove(&Subject::Candidate)
            .ok_or_else(|| invalid_data("missing candidate benchmark result"))?;
        let evaluation = if baseline_valid && candidate_valid {
...
fn execute(spec: CrateRunSpec<'_>) -> Result<()> {
    ...
    let completed_environment = EnvironmentSnapshot::capture(artifact_root)?;
