...
impl EnvironmentSnapshot {
    pub(super) fn capture(disk_path: &Path) -> Result<Self> {
        ...
        let inherited_environment = env::vars_os().collect::<Vec<_>>();
        let (inherited_environment_names, inherited_environment_sha256) =
            inherited_environment_binding(inherited_environment.iter().cloned())?;
    ...
    pub(super) fn drift_reasons(&self, completed: &Self) -> Vec<String> {
        ...
        compare(
            ...
            self.inherited_environment_names == completed.inherited_environment_names,
        );
        compare(
            "inherited environment digest",
            self.inherited_environment_sha256 == completed.inherited_environment_sha256,
        );
        compare(
...
fn inherited_environment_binding(
    ...
    Ok((names, lower_hex(digest.finalize())))
}
